            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {/* Flat lessons fallback if no topics */}
                        {topics.length === 0 && allLessons.length > 0 && (
                            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                                ...
                                <div className="space-y-2">
                                    {allLessons.map((lesson, li) => (
                                        <div key={li} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                                            <div className="w-7 h-7 rounded-full bg-[#021d49] text-white text-xs font-bold flex items-center justify-center flex-shrink-0">{li + 1}</div>
                                            <p className="text-sm font-semibold text-gray-900 flex-1">{lesson.title}</p>
                                            {lesson.duration && <span className="text-xs text-gray-500">{lesson.duration}</span>}
                                        </div>
                                    ))}
                                </div>
                        ...
                        {finalAssessment && (
                            <div className="bg-white rounded-2xl border border-indigo-200 shadow-sm overflow-hidden">
                                <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-4 flex items-center gap-3">
                                    ...
                                    <span className="ml-auto text-indigo-200 text-sm">{finalAssessment.questions?.length || 0} questions</span>
                                </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                            ...
                            <div className="space-y-3 text-sm">
                                {[
                                    { icon: Icons.BookOpen, label: 'Topics', value: topics.length || '—' },
                                    { icon: Icons.FileText, label: 'Total Lessons', value: allLessons.length || topics.reduce((acc, t) => acc + (t.lessons?.length || 0), 0) || '—' },
