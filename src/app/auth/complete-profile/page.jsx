...
import { Eye, EyeOff, Lock, Mail, CheckCircle, AlertCircle, ShieldCheck } from 'lucide-react';
...
function CompleteProfileForm() {
  ...
  useEffect(() => {
    if (!token) {
      setError('Invalid setup link. Please contact support to get a new invitation.');
    }
  }, [token]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token) return setError('Invalid setup link. Please contact support.');
    if (form.password.length < 8) return setError('Password must be at least 8 characters');
    if (form.password !== form.confirmPassword) return setError('Passwords do not match');

    setLoading(true);
    try {
      await authService.setInitialPassword({
        token,
      ...
      if (msg.toLowerCase().includes('expired') || msg.toLowerCase().includes('invalid')) {
        setError('This setup link has expired or is invalid. Please contact support for a new invitation.');
      } else {
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-gray-500 max-w-xs mx-auto">
            Welcome to the Arin Fellowship! Set a personal password to secure your account.
          </p>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-700 flex gap-3">
          ...
          <span>
            Choose a new password for your account. You will use this to log in going forward.
          </span>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 space-y-5">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={email}
                  readOnly
                  className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg text-sm bg-gray-50 text-gray-600 cursor-default focus:outline-none"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading || !token || form.password.length < 8 || form.password !== form.confirmPassword}
