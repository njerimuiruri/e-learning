...
export function useDraft(
    ...
    (async () => {
      ...
      try {
        ...
        if (httpStatus !== 404) {
          ...
          console.warn(
            `[useDraft] Could not load draft from DB (${httpStatus ?? "network"}):`,
            msg,
          );
        }
      }
      ...
      try {
        ...
        if (raw) {
          ...
          if (parsed?.data) {
            console.info(
              `[useDraft] Migrating localStorage draft "${draftKey}" → DB`,
            );
            ...
            const resolvedTitle =
              (typeof parsed.data?.title === "string"
                ? parsed.data.title
                : null) ||
              (typeof parsed.data?.moduleData?.title === "string"
                ? parsed.data.moduleData.title
                : null) ||
              "Untitled";
            try {
              ...
              console.info(
                `[useDraft] Migration complete  localStorage draft removed`,
              );
            } catch (saveErr) {
              console.warn(
                `[useDraft] Migration to DB failed, keeping localStorage copy`,
                saveErr,
              );
            }
            if (!cancelled) {
  ...
  const performSave = useCallback(async () => {
    ...
    const resolvedTitle =
      ...
      (typeof snapshot?.moduleData?.title === "string"
        ? snapshot.moduleData.title
        : null) ||
      "Untitled";
    ...
    try {
      ...
      console.error(
        `[useDraft] DB save failed (${httpStatus ?? "network"}):`,
        msg,
        err,
      );
      ...
      const reason =
        httpStatus === 401
          ...
          : httpStatus === 413
            ? "Draft too large  images may need to be re-uploaded."
            : httpStatus
              ? `Server error (${httpStatus})  please try again.`
  ...
  const saveDraft = useCallback(
    async (overrideEntityId) => {
      if (timerRef.current) clearTimeout(timerRef.current);
      if (overrideEntityId !== undefined)
        entityIdRef.current = overrideEntityId;
      await performSave();
    },
    [performSave],
  );
  ...
  const discardDraft = useCallback(async () => {
    if (timerRef.current) clearTimeout(timerRef.current);
    try {
      await draftService.discard(draftKey);
    } catch {}
    setSavedAt(null);
    setHasDraft(false);
    setLoadedDraft(null);
