...
function Confetti({ active }) {
    ...
    // Inject styles once and manage lifecycle properly
    useEffect(() => {
        if (!active) {
            ...
            if (styleRef.current && styleRef.current.parentNode) {
                try {
                    ...
                    // Already removed or detached, silent fail
                    console.debug('[Confetti] Style cleanup failed (likely already removed):', e?.message);
                }
      ...
      `;
            ...
            console.warn('[Confetti] Failed to inject styles:', e?.message);
        }
        ...
        return () => {
            ...
            if (styleRef.current && styleRef.current.parentNode) {
                try {
                    ...
                    // Already removed or detached, silent fail
                    console.debug('[Confetti] Unmount cleanup failed (likely already removed):', e?.message);
                }
            }
            styleRef.current = null;
        };
    ...
    return (
        <div ref={containerRef} className="fixed inset-0 pointer-events-none overflow-hidden" style={{ zIndex: 9999 }}>
    ...
    return (
        <div className="flex items-center gap-1.5">
            {Array.from({ length: max }, (_, i) => (
                <span
                    key={i}
                    className={`w-3 h-3 rounded-full border-2 transition-colors ${i < used
...
export default function QuizResultsModal({
    ...
    return (
        <>
            ...
            {/* Backdrop */}
            <div
                className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-[900] transition-opacity duration-300 ${visible ? 'opacity-100' : 'opacity-0'
                    }`}
            />
            ...
            ${darkMode ? 'bg-gray-900 text-white' : 'bg-white'}`}
                >
                    {/* ── Top colour band ── */}
                    <div
                        className={`px-8 pt-10 pb-8 text-center ${passed
                    ...
                    >
                        {/* Icon badge */}
                        <div
                            className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-5 shadow-lg ${passed
                    ...
                    <div className={`px-8 pb-8 space-y-5 ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
                        ...
                        {!passed && !lessonResetRequired && maxAttempts && (
                            <div className={`flex items-center justify-between rounded-xl p-4 ${darkMode ? 'bg-amber-900/20 border border-amber-800' : 'bg-amber-50 border border-amber-200'
                                }`}>
                                <div>
                        ...
                        {lessonResetRequired && (
                            <div className={`flex items-start gap-3 rounded-xl p-4 ${darkMode ? 'bg-red-900/20 border border-red-800' : 'bg-red-50 border border-red-200'
                                }`}>
                        ...
                        <div className="space-y-3 pt-2">
                            ...
                            {!passed && (
                                <button
                                    onClick={onReturnToLesson}
                                    className={`w-full py-3 rounded-xl font-semibold text-sm transition-all border ${darkMode
